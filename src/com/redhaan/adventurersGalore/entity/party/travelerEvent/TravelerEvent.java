...
public class TravelerEvent extends GameObject {
	...
	public ArrayList<DialogueText> introTexts;
	public ArrayList<DialogueText> outcomeTexts;
	...
	public TravelerEvent() {
		...
		introTexts = new ArrayList<DialogueText>();
		outcomeTexts = new ArrayList<DialogueText>();
	...
	public static void rollTravelerEvent() {
		...
		default:
			System.out.println("Error in rollTravelerEvent");
			break;
	...
	public void update(GameContainer gameContainer, float deltaTime) {
		...
		if (textFinished && introText) {
			...
			if (gameContainer.getInput().getMouseX() > optionsOffX
					&& gameContainer.getInput().getMouseX() < optionsOffX + optionsWidth) {
				if (gameContainer.getInput().getMouseY() > optionAOffY
						&& gameContainer.getInput().getMouseY() < optionAOffY + optionsHeight) {
					hoverOption = 1;
			...
			if (gameContainer.getInput().getMouseX() > optionsOffX
					&& gameContainer.getInput().getMouseX() < optionsOffX + optionsWidth) {
				if (gameContainer.getInput().getMouseY() > optionBOffY
						&& gameContainer.getInput().getMouseY() < optionBOffY + optionsHeight) {
					hoverOption = 2;
					if (gameContainer.getInput().isButtonUp(MouseEvent.BUTTON1)) {
						if (introText) {
			...
			if (gameContainer.getInput().getMouseX() > optionsOffX
					&& gameContainer.getInput().getMouseX() < optionsOffX + optionsWidth) {
				if (gameContainer.getInput().getMouseY() > optionCOffY
						&& gameContainer.getInput().getMouseY() < optionCOffY + optionsHeight) {
					hoverOption = 3;
					if (gameContainer.getInput().isButtonUp(MouseEvent.BUTTON1)) {
						if (introText) {
			...
			if (gameContainer.getInput().getMouseX() > optionsOffX
					&& gameContainer.getInput().getMouseX() < optionsOffX + optionsWidth) {
				if (gameContainer.getInput().getMouseY() > optionDOffY
						&& gameContainer.getInput().getMouseY() < optionDOffY + optionsHeight) {
					hoverOption = 4;
					if (gameContainer.getInput().isButtonUp(MouseEvent.BUTTON1)) {
						if (introText) {
			...
			if (gameContainer.getInput().getMouseY() > 350 && gameContainer.getInput().getMouseY() < 370) {
				if (gameContainer.getInput().isButtonUp(MouseEvent.BUTTON1)) {
					if (!textFinished) {
						...
						stringToDrawFrom = "";
						if (introText) {
							if (paragraphNumber >= currentEvent.introTexts.size()) {
								textFinished = true;
							}
						} else if (outcomeText) {
							if (paragraphNumber >= currentEvent.outcomeTexts.size()) {
								outcomeText = false;
								paragraphFinished = false;
								paragraphNumber = 0;
								timer = 0;
		...
		// if(introText) { if (paragraphNumber >= currentEvent.introTexts.size()) {
		// textFinished = true; } }
		// else if (outcomeText) { System.out.println(paragraphNumber); if
		// (paragraphNumber >= currentEvent.outcomeTexts.size()) { textFinished = true;
		// } }
		...
		if (!textFinished) {
			...
			if (stringToDrawFrom.length() == 0) {
				if (introText) {
					stringToDrawFrom = currentEvent.introTexts.get(paragraphNumber).text;
				} else if (outcomeText) {
					stringToDrawFrom = currentEvent.outcomeTexts.get(paragraphNumber).text;
				}
			}
	...
	public void render(GameContainer gameContainer, Renderer renderer) {
		...
		if (textFinished && introText)
			renderer.drawRectOpaque(445, 100, optionsWidth, optionsHeight, 0xdd775533);
		if (textFinished && introText)
			renderer.drawRectOpaque(445, 150, optionsWidth, optionsHeight, 0xdd774422);
		if (textFinished && introText)
			renderer.drawRectOpaque(445, 200, optionsWidth, optionsHeight, 0xdd775522);
		if (textFinished && introText)
			renderer.drawRectOpaque(445, 250, optionsWidth, optionsHeight, 0xdd774433);

		if (textFinished && introText && hoverOption == 1)
			renderer.drawRect(444, 99, optionsWidth + 2, optionsHeight + 2, 0xffFFCC88);
		if (textFinished && introText && hoverOption == 2)
			renderer.drawRect(444, 149, optionsWidth + 2, optionsHeight + 2, 0xffFFCC88);
		if (textFinished && introText && hoverOption == 3)
			renderer.drawRect(444, 199, optionsWidth + 2, optionsHeight + 2, 0xffFFCC88);
		if (textFinished && introText && hoverOption == 4)
			renderer.drawRect(444, 249, optionsWidth + 2, optionsHeight + 2, 0xffFFCC88);

		if (textFinished && introText)
			renderer.drawText(currentEvent.optionAText, GameManager.GAMEWIDTH / 2 + 130,
					GameManager.GAMEHEIGHT / 2 - 120, 0xff451205);
		if (textFinished && introText)
			renderer.drawText(currentEvent.optionBText, GameManager.GAMEWIDTH / 2 + 130,
					GameManager.GAMEHEIGHT / 2 - 70, 0xff451205);
		if (textFinished && introText)
			renderer.drawText(currentEvent.optionCText, GameManager.GAMEWIDTH / 2 + 130,
					GameManager.GAMEHEIGHT / 2 - 20, 0xff451205);
		if (textFinished && introText)
			renderer.drawText(currentEvent.optionDText, GameManager.GAMEWIDTH / 2 + 130,
					GameManager.GAMEHEIGHT / 2 + 30, 0xff451205);

		if (!textFinished)
			renderer.drawRectOpaque(450, 350, 50, 20, 0xffAA9933);
		if (textFinished && outcomeText) {
			renderer.drawRectOpaque(450, 350, 50, 20, 0xffAA9933);
		}

		if (outcomeText && paragraphNumber + 1 >= currentEvent.outcomeTexts.size()) {
			renderer.drawText("Finish", 460, 355, 0xff443012);
		} else if (paragraphFinished && !textFinished) {
			renderer.drawText("Next", 460, 355, 0xff443012);
		} else if (!textFinished) {
			renderer.drawText("Skip", 460, 355, 0xff443012);
		}
		...
		if (firstLine.length() > 0) {
			renderer.drawText(firstLine, 140, 320, 0xffFFFFFF);
		}
		if (secondLine.length() > 0) {
			renderer.drawText(secondLine, 140, 335, 0xffFFFFFF);
		}
		if (thirdLine.length() > 0) {
			renderer.drawText(thirdLine, 140, 350, 0xffFFFFFF);
		}
		if (fourthLine.length() > 0) {
			renderer.drawText(fourthLine, 140, 365, 0xffFFFFFF);
		}

		if (introText && !textFinished) {
			if (currentEvent.introTexts.get(paragraphNumber).name != null) {
				renderer.drawImageTile(currentEvent.introTexts.get(paragraphNumber).speakerIcon, 80, 325,
						currentEvent.introTexts.get(paragraphNumber).xTile,
						currentEvent.introTexts.get(paragraphNumber).yTile);
				renderer.drawText(currentEvent.introTexts.get(paragraphNumber).name, 80, 365, 0xffDDDDBB);
			}

		} else if (outcomeText && !textFinished) {
			if (currentEvent.outcomeTexts.get(paragraphNumber).name != null) {
				renderer.drawImageTile(currentEvent.outcomeTexts.get(paragraphNumber).speakerIcon, 80, 325,
						currentEvent.outcomeTexts.get(paragraphNumber).xTile,
						currentEvent.outcomeTexts.get(paragraphNumber).yTile);
				renderer.drawText(currentEvent.outcomeTexts.get(paragraphNumber).name, 80, 365, 0xffDDDDBB);
			}
		}
	...
	public void writeText(String text, Renderer renderer) {
		...
		for (int i = 0; i < strings.size(); i++) {
			renderer.drawText(strings.get(i), GameManager.GAMEWIDTH / 2 - 190,
					(GameManager.GAMEHEIGHT / 2 + 75) + (i * 15), 0xffddddaa);
		}
	}
