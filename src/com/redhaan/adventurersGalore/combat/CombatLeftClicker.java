...
public abstract class CombatLeftClicker {
	...
	public static PlayerTurnLeftClickSituations decidePlayerLeftClickSituation(Adventurer adventurer, int x, int y) {
		...
		adventurer.moveRange = MoveRangeFiller.fillMoveRange(adventurer.getCombatX(), adventurer.getCombatY(), adventurer.currentStats.move);
		...
		if(clickedLocation[0] == adventurer.getCombatX() && clickedLocation[1] == adventurer.getCombatY()) {
			...
			else { return PlayerTurnLeftClickSituations.NothingToDo; }
		}
			
		//Clicked in MoveRange	
		if(adventurer.moving) {
			for (int i = 0; i < adventurer.moveRange.size(); i++) {
			if (clickedLocation[0] == adventurer.moveRange.get(i)[0] && clickedLocation[1] == adventurer.moveRange.get(i)[1]) {
		...
		for (int i = 0; i < Combat.enemies.size(); i++) {
			...
			if (clickedLocation[0] == Combat.enemies.get(i).getCombatX() && clickedLocation[1] == Combat.enemies.get(i).getCombatY()) {
				...
				if(checkIfEnemyInWeaponRange(adventurer, clickedLocation)) {
					...
					else { return PlayerTurnLeftClickSituations.Enemy_NOTINRange; }
				}					
			}
		}
